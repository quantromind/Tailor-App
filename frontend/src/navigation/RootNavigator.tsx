...
// Mirrors the same logic as api/config.ts so the debug ping hits the right host
const getDevHost = (): string => {
  ...
  const debuggerHost: string | undefined =
    manifest?.hostUri || manifest?.debuggerHost;
  return debuggerHost ? debuggerHost.split(':')[0] : '192.168.1.6';
};
...
import MyDesignsScreen from '../../app/screens/design/MyDesignsScreen';
...
export default function RootNavigator() {
  ...
  useEffect(() => {
    checkInitialState();
    // Test API — use dynamic host so this works across IP changes
    fetch(`http://${getDevHost()}:5000/api/debug/test`)
      .then(async (res) => {
        if (!res.ok) {
  ...
  // Listen for forced logouts triggered by the 401 interceptor in api/config.ts
  useEffect(() => {
    const subscription = addLogoutListener(handleLogout);
      ...
      >
        {!hasSelectedLanguage ? (
          ...
          <Stack.Group screenOptions={{ headerShown: false }}>
            ...
            <Stack.Screen name="MyDesigns" component={MyDesignsScreen} options={{ animation: 'slide_from_right' }} />
