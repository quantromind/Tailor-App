...
export default function AddDesignScreen({ navigation }: any) {
  ...
  const handlePickImage = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      ...
      allowsEditing: true,
      aspect: [4, 3],
      quality: 0.3,   // Low quality to keep base64 payload small
      base64: true,
      exif: false,
    });
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScreenHeader title="Add Your Design" subtitle="Custom Atelier Piece" onBack={() => navigation.goBack()} />
      ...
      <KeyboardAwareScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} enableOnAndroid={true} keyboardOpeningTime={0}>
        <View style={styles.section}>
          ...
          <TextInput 
            ...
            placeholderTextColor={Colors.textLight} 
          />
          <AnimatedPressable onPress={handlePickImage} style={styles.imagePickerBtn} scaleTo={0.98}>
            {image ? (
        ...
        <Button label={loading ? 'Saving...' : 'Save Design'} icon="checkmark-circle-outline" onPress={handleSaveDesign} loading={loading} style={{ marginTop: 10 }} />
