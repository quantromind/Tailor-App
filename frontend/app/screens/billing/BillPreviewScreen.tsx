...
import { createCustomer, createCustomDesign, createOrder, createBillPaymentOrder, verifyPayment, reportPaymentFailure } from '../../../api';
...
export default function BillPreviewScreen({ route, navigation }: any) {
  ...
  // Helper to save order to backend & local storage
  const saveOrder = async (paymentData?: { paymentStatus: 'unpaid' | 'paid' | 'failed'; paymentId: string; razorpayOrderId: string }) => {
    ...
    // 4. Create Order in Backend (with optional payment data)
  ...
  const handlePayment = async () => {
    ...
    try {
      const amount = parseFloat(billData.price);
      if (!amount || amount <= 0) {
      ...
      const razorpayOrder = await createBillPaymentOrder(amount, `${billData.item} - ${billData.clientName}`);
      currentOrderId = razorpayOrder.orderId;

      // 2. Open Razorpay Checkout
      const options = {
        description: `Payment for ${billData.item} - ${billData.clientName}`,
      ...
      if (!RazorpayCheckout || typeof RazorpayCheckout.open !== 'function') {
        console.warn('[Billing] Razorpay native module is not available (running in Expo Go). Falling back to mock checkout.');
      ...
      if (verification.success) {
        // 4. Save order with payment data
        await saveOrder({
          paymentStatus: 'paid',
        ...
        Alert.alert(
          '✅ Payment Successful',
          `Payment of ₹${billData.price} received and order saved successfully!`,
      ...
      if (error?.code === 'PAYMENT_CANCELLED' || error?.description?.includes('cancelled')) {
        ...
        Alert.alert(
          'Payment Error',
          error?.description || error?.message || 'Something went wrong during payment. Please try again.'
        );
      }

      // Log the failed/cancelled attempt on the backend for audit purposes.
      if (currentOrderId) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScreenHeader title="Atelier Design" subtitle={t('receipt_header')} onBack={() => navigation.goBack()} />
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.footerActions}>
          ...
          <AnimatedPressable onPress={handlePayment} style={styles.actionBtn} disabled={isProcessing}>
            <LinearGradient colors={['#2563EB', '#1E40AF']} style={styles.actionGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              {isProcessing ? (
                <ActivityIndicator color="#FFFFFF" size="small" />
              ) : (
                <Ionicons name="card-outline" size={20} color="#FFFFFF" />
              )}
              <Text style={styles.actionText}>{isProcessing ? 'Processing...' : 'Collect Payment via Razorpay'}</Text>
            </LinearGradient>
          </AnimatedPressable>

          {/* Save Without Payment Button */}
          ...
          <View style={styles.shareRow}>
            <AnimatedPressable style={[styles.shareBtn, { backgroundColor: '#25D366' }]} onPress={handleShareWhatsApp} scaleTo={0.95}>
              <Ionicons name="logo-whatsapp" size={18} color="#fff" />
              <Text style={styles.shareBtnText}>{t('whatsapp')}</Text>
            </AnimatedPressable>
            <AnimatedPressable style={[styles.shareBtn, { backgroundColor: '#EA4335' }]} onPress={handleGmail} scaleTo={0.95}>
          ...
          <View style={styles.shareRow}>
            <AnimatedPressable style={[styles.shareBtn, { backgroundColor: '#6B705C' }]} onPress={handleSharePDF} scaleTo={0.95}>
              <Ionicons name="document-text-outline" size={18} color="#fff" />
              <Text style={styles.shareBtnText}>{t('share_pdf')}</Text>
            </AnimatedPressable>
            <AnimatedPressable style={[styles.shareBtn, { backgroundColor: '#344E41' }]} onPress={handlePrint} scaleTo={0.95}>
