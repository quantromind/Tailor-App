...
export default function KidsCategoryScreen({ route, navigation }: any) {
  const { t } = useTranslation();
  const { client } = route.params || {};

  const categories = [
    {
      title: t('kidswear_title'),
      items: [
        { id: 'k_frock', name: t('frock_cutting'), type: 'Shirt', icon: 'shirt-outline' },
        { id: 'k_suit', name: t('boys_suit'), type: 'Shirt', icon: 'briefcase-outline' },
        { id: 'k_kurta', name: t('kurta_pyjama'), type: 'Shirt', icon: 'shirt-outline' },
      ],
    },
  ];

  const handleSelect = (item: any) => {
    // For kids, most are currently handled as Shirt type (upper body)
    // In future, can split into Pant and Shirt as needed
    if (item.type === 'Pant') navigation.navigate('PantMeasurement', { item, gender: 'kids', client });
    else navigation.navigate('ShirtMeasurement', { item, gender: 'kids', client });
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>{t('kidswear_title')}</Text>
        <View style={{ width: 32 }} />
      </LinearGradient>
                ...
                >
                  <View style={styles.imageContainer}>
                    <View style={styles.iconPlaceholder}>
                      <Ionicons name={item.icon as any || 'happy-outline'} size={48} color={Colors.primary} />
                    </View>
                  </View>
