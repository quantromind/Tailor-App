...
import { StatusBadge, toneForOrderStatus } from '../../../src/components/ui/StatusBadge';
      ...
      <View style={{ flex: 1 }}>
        ...
        <KeyboardAwareScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} enableOnAndroid={true} keyboardOpeningTime={0}>
          ...
          {sortedOrders.length === 0 && (
            <View style={styles.card}>
              <EmptyState icon="shirt-outline" title="No orders yet" subtitle="Orders placed for this client will appear here." compact />
            </View>
          )}
          {sortedOrders.map((order) => (
            <View key={order.id} style={[styles.card, targetOrderId === order.id && styles.highlightedCard]}>
              ...
              <View style={styles.infoRow}>
                ...
                <StatusBadge label={order.status === 'Completed' ? t('filter_completed') : t('filter_pending')} tone={toneForOrderStatus(order.status)} />
              </View>
              {order.deliveryDate && (
                <View style={styles.infoRow}>
