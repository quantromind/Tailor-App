...
import { calculatePrice } from '../../../src/utils/priceCalculator'; // Forced refresh
...
export default function PantMeasurementScreen({ route, navigation }: any) {
  ...
  const handleCalculateBill = () => {
    ...
    navigation.navigate('BillPreview', {
      billData: { 
        clientName, phone, item: item.name, type: 'Pant', gender: route.params?.gender || 'male', 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={{ flex: 1 }}>
      <ScreenHeader title={`${item.name} ${t('finalize_design')}`} onBack={() => navigation.goBack()} />
      ...
      <KeyboardAwareScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" enableOnAndroid={true} keyboardOpeningTime={0}>
        ...
        <View style={styles.section}>
          ...
          {!showAddFields ? (
            <AnimatedPressable onPress={() => setShowAddFields(true)} style={styles.showAddBtn} scaleTo={0.95}>
        ...
        <Button label={t('finalize_design')} icon="sparkles-outline" onPress={handleCalculateBill} style={{ marginTop: 10 }} />
